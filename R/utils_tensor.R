...
mat <- function(X, mode) {
	d <- dim(X)
	if (any(d == 0)) {
		cli::cli_abort("Cannot unfold empty array (dimension 0).")
	}
...
#'
#' @description Multiplies an array by a matrix along a given mode
#' @param A Array
#' @param M Matrix
#' @param k Mode
...
#' @param modes Modes for multiplication
...
#' @return Kronecker product A otimes B
#' @keywords internal
