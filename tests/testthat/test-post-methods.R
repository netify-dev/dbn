...
test_that("net_snapshot handles sparse networks", {
	...
	results_small <- list(
		...
		dims = list(m = n_small, n_row = n_small, n_col = n_small, p = 1, Tt = Tt, is_bipartite = FALSE)
	)
	...
	results_large <- list(
		...
		dims = list(m = n_large, n_row = n_large, n_col = n_large, p = 1, Tt = Tt, is_bipartite = FALSE)
	)
