...
test_that("verbose defaults are set correctly", {
	...
	expect_equal(formals(dbn_static)$verbose, TRUE)
	expect_equal(formals(dbn_dynamic)$verbose, TRUE)
	expect_equal(formals(dbn_lowrank)$verbose, TRUE)
	expect_equal(formals(dbn_hmm)$verbose, TRUE)
})