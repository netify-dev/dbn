...
test_that("data can be loaded from file path", {
	...
	Y = simulate_test_data(n = 5, p = 2, time = 10, seed = 6886)
...
test_that("verbose parameter controls output", {
	Y = simulate_test_data(n = 10, p = 2, time = 10, seed = 6886)
...
test_that("thinning works correctly", {
	Y = simulate_test_data(n = 10, p = 2, time = 10, seed = 6887)
...
test_that("dynamic model initializes and runs", {
	skip_on_cran()
	data = simulate_dynamic_dbn(n = 8, p = 2, time = 10, seed = 6889)
	...
	if (!is.null(results)) {
		...
		skip("dynamic model did not converge with these settings")
	}
})
