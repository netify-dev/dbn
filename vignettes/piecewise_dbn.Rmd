...
This example requires the `peacesciencer` package and its external data. To run it locally:
...
The 2008 financial crisis provides a natural break point. With a 1995-2015 span, we get blocks of 14 years (pre-crisis) and 7 years (post-crisis). For networks of this size, use `store_theta = FALSE` to reduce memory usage:
...
Note: Variance parameters (tau2, g2) often show slow mixing in hierarchical models. For publication-quality inference, consider longer chains (nscan = 5000+).
...
A high probability that the difference exceeds threshold (e.g., `prob > 0.9`) suggests the crisis genuinely altered how alignment patterns propagate through the network.
